...
if ($patientId && $prescriptionId) {
    ...
    $stmt = $conn->prepare("SELECT treatment_plans, images FROM tbl_ptp WHERE patient_id = ? AND procedure_id = ?"); // Updated patient_prescription to prescription_id
...
<textarea id="treatment-plans" name="treatment-plans" class="form-textarea" placeholder="Describe the proposed treatment plans here..." rows="5" style="padding: 10px; border: 1px solid #ccc; border-radius: 5px; font-size: 14px; width: auto; margin-left: 0;" required readonly>
    <?php echo htmlspecialchars($treatmentPlans); ?>
        ...
        <?php
            ...
            if (!is_array($images)) {
                echo "<p>Error: Images not decoded properly.</p>";
                $images = [];
            }
            ...
            if (!empty($images)) {
                foreach ($images as $image) {
                    ...
                    $imagePath = htmlspecialchars(trim($image)); 
                    
                    echo '<div style="width: 120px; text-align: center;">';
                    echo '<img src="' . $imagePath . '" style="width: 100px; height: 100px; cursor: pointer; object-fit: cover;" onclick="openModal(\'' . $imagePath . '\')">';
                    echo '</div>';
                }
            } else {
