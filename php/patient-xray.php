...
if ($patientId && $prescriptionId) {
    ...
    // Fetch image paths from tbl_patientxray
    $stmt = $conn->prepare("SELECT image_paths FROM tbl_patientxray WHERE patient_id = ? AND prescription_id = ?"); // Updated from patient_prescription to prescription_id
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<script>alert('No images found for this patient.');</script>";
        $imagePaths = [];
    }
...
<h2 class="info-title" style="margin-bottom: 10px; margin-left:20px; margin-top:-20px;">Patient Xray</h2>
    ...
    <div id="uploaded-images-container" class="images-container">
        ...
        if (!empty($imagePaths)) {
            foreach ($imagePaths as $imagePath) {
                ...
                echo '<img src="' . htmlspecialchars($imagePath) . '" style="width: 100px; height: 100px; cursor: pointer; object-fit: cover;" onclick="openModal(\'' . htmlspecialchars($imagePath) . '\')">';
                echo '</div>';
            }
        } else {
