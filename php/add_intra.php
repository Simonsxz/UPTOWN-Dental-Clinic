...
<body>
    ...
    <h2 class="info-title" style="text-align: center; margin-bottom: 5px; font-family: Arial, sans-serif; color: #333;">Intra Oral Photos</h2>
    
    <!-- Image upload input -->
    <form class="details-form-intra" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 15px;">
        <label for="intra-image-upload" style="font-size: 14px; font-weight: bold; color: #555;">Upload Intra Oral Images (Optional):</label>
        <input id="intra-image-upload" type="file" accept="image/*" multiple style="padding: 10px; border: 1px solid #ccc; border-radius: 5px; font-size: 14px;" onchange="displayIntraImages(event)">

        <!-- Container to display uploaded images -->
        <div id="intra-uploaded-images-container" style="margin-top: 20px; display: flex; flex-wrap: wrap; gap: 15px;">
...
<div id="intra-image-modal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.8); justify-content: center; align-items: center; z-index: 1000; flex-direction: column; padding: 20px; box-sizing: border-box;">
    <img id="intra-modal-image" src="" alt="Large View" style="max-width: 90%; max-height: 80%; border: 5px solid white; margin-bottom: 20px; border-radius: 10px;">
    <a id="intra-download-link" href="" download style="background-color: #007BFF; color: white; text-decoration: none; padding: 10px 20px; border-radius: 5px; font-size: 14px; margin-bottom: 10px;">Download</a>
    <button onclick="closeIntraModal()" style="background-color: #dc3545; color: white; border: none; padding: 10px 20px; border-radius: 5px; font-size: 14px; cursor: pointer;">Close</button>
    <button onclick="removeIntraImage()" style="background-color: #28a745; color: white; border: none; margin-top:5px; padding: 10px 20px; border-radius: 5px; font-size: 14px; cursor: pointer;">Remove Image</button>
</div>
...
<script>
    ...
    function displayIntraImages(event) {
        ...
        files.forEach(file => {
            ...
            reader.onload = function (e) {
                const imageData = e.target.result; // This is the base64 data URL of the image

                // Add new image data URL to the intraOralSelectedFiles array
                intraOralSelectedFiles.push({
                    dataUrl: imageData,
                    name: file.name,
                    date: currentDate
                });
    ...
    function displayStoredIntraImages() {
        ...
        intraOralSelectedFiles.forEach((image, index) => {
            ...
            img.onclick = function () {
                openIntraModal(image.dataUrl, image.name);
            };
            ...
            intraUploadedImagesContainer.appendChild(imageWrapper);
        });
    }

    // Function to open modal with a large image for intra oral photos
    function openIntraModal(imageSrc, fileName) {
        const modal = document.getElementById('intra-image-modal');
        const modalImage = document.getElementById('intra-modal-image');
        const downloadLink = document.getElementById('intra-download-link');
    ...
    // Close the modal for intra oral images
    function closeIntraModal() {
        const modal = document.getElementById('intra-image-modal');
        modal.style.display = 'none';
    }

    // Remove the image from the container for intra oral images
    function removeIntraImage() {
        const modalImage = document.getElementById('intra-modal-image');
        const intraUploadedImagesContainer = document.getElementById('intra-uploaded-images-container');

        // Find the image container to remove
        const imageContainers = intraUploadedImagesContainer.children;
        for (let i = 0; i < imageContainers.length; i++) {
            const imgElement = imageContainers[i].querySelector('img');
            if (imgElement && imgElement.src === modalImage.src) {
                intraUploadedImagesContainer.removeChild(imageContainers[i]);
                intraOralSelectedFiles.splice(i, 1); // Remove the file from the selectedFiles array
                break;
            }
        }
    ...
    document.getElementById("saveIntraButton").addEventListener("click", function () {
        // Collect all uploaded images for intra oral
        const uploadedIntraImages = intraOralSelectedFiles.map(file => file.dataUrl); // Extract only the data URL
        ...
        // Prepare the form data for intra oral photos
        const formData = new FormData();
        formData.append('patient_id', patientId);
        formData.append('prescription_id', prescriptionId);

        // Add all uploaded images to the form data if there are any
        if (uploadedIntraImages.length > 0) {
            uploadedIntraImages.forEach((fileDataUrl, index) => {
                formData.append(`intra_image_${index}`, fileDataUrl);
            });
        }
        ...
        Swal.fire({
            ...
            text: 'Please confirm if you want to proceed with saving your photos.',
            icon: 'question',
                    ...
                    .then((data) => {
                        if (data.success) {
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Error!',
                                text: data.message || 'Failed to save photos.',
                            });
                        }
                    })
                    .catch((error) => {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Error!',
                            text: 'An error occurred while saving your photos.',
                        });
                    });
            }
        });
